...
#define st_clk 2 // ST时钟为2M，则2个计数为1us
    ...
    TK_Handle.Init.ShortswNum = TK_SHORTSW_NUM_0;
...
/*	低功耗初始化	*/
    ...
    TK_Handle.Lp.RtcInt = 0;                       // RTC_CRH_ALRIE;          // 需要使能的RTC中断
    TK_Handle.Lp.Alarm_tick =
        10; // 使能闹钟中断后定时唤醒MCU的时间 = TK_Handle.Lp.LpWakeTime * (TK_Handle.Lp.Alarm_tick + 1)
#endif
#if APP_ADC_ENABLE
    /*	等待序列转换完成*/
