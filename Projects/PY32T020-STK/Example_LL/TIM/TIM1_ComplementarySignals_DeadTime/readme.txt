...
================================================================================
功能描述：
此样例实现了定时器的刹车功能，CH1和CH1N互补pwm输出，接收到外部IO口的刹车信号（输
入低电平）后，PWM信号关闭，由于BDTR.AOE置位，所以刹车信号取消（输入高电平）后，
继续pwm输出，此样例实现了死区功能。
...
complementary pwm outputs.After receiving the brake signal (input low level)from
the external IO port, the PWM signal is turned off. Because BDTR.AOE is set, the 
pwm output continues after the brake signal is cancelled (input high level). 
This example realizes the dead zone function.
